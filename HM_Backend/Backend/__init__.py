...
class Connect_to_Frontend:
    def __init__(self, security):
        ...
        # 환경변수에서 호스트/포트/디버그 설정을 읽어온다 (기본값 존재)
        host = os.getenv('BACKEND_HOST', '0.0.0.0')
        try:
            port = int(os.getenv('BACKEND_PORT', '5000'))
        except Exception:
            port = 5000
        debug_env = os.getenv('FLASK_DEBUG', 'true')
        debug = str(debug_env).lower() in ('1', 'true', 'yes', 'y')

        if security:
            load_dotenv()
            ssl_cert = os.getenv('SSL_CERT')
            ssl_key = os.getenv('SSL_KEY')
            ssl_pw = os.getenv('SSL_PW')
            ...
            if ssl_cert and ssl_key:
                ...
                self.app.run(ssl_context=ssl_context, host=host, port=port, debug=debug)
            else:
                # 인증서가 없으면 보안 모드 요청이어도 일반 HTTP로 실행
                self.app.logger.warning('SSL 인증서/키가 설정되지 않아 보안 모드를 비활성화하고 HTTP로 실행합니다.')
